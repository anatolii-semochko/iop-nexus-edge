...
- Single value, `Bool`. `true` heats, `false` is idle.
- Ordinary actuator - has a normal `AUTO`/`MANUAL` mode via the Dual
  ...
  sensor.
- Must never be active at the same time as `../cooler` - enforced by the
...
full story. Its own forbidden-state rule against Cooler used to live on
...
cross-*device* rule), since Heater and Cooler are two separate devices now.
