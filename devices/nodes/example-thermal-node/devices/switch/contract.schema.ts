/**
 * Switch - atomic manual-input device on the example-thermal-node
 * (AGENTS.md section 7/30). A single boolean input/output with no wiring
 * to any process (apps/orchestrator) - kept deliberately unused, per an
 * explicit instruction to preserve all four original devices
 * (temperature/heater/cooler/switch) rather than dropping the one with no
 * current consumer.
 *
