...
export async function deviceRoutes(app: FastifyInstance): Promise<void> {
  ...
  // Write path: proxies to EdgeX core-command. Used today by the dev
  // simulator page to override a virtual device's sensor values.
  app.put<{ Params: { id: string; resource: string }; Body: { value: unknown } }>(
    ...
    async (request, reply) => {
      ...
      await writeResource(device.edgex_device_name, request.params.resource, request.body.value);
      return { status: "ok" };
    },
  );
}
